...
func (c *HTTPClient) Aggregate(ctx context.Context, aggRequest *types.AggregateRequest) error {
	b, err := json.Marshal(aggRequest)
	if err != nil {
		return err
	}
	endpoint := fmt.Sprintf("%s/%s", c.Endpoint, "invoice")
